...
for x in participants:
    ...
    for sessions in glob.glob("%s/*" %x): #Store list of sessions for current participant
        for files in glob.glob("%s/*" %sessions):
            ...
            for x in range(int(math.ceil(len((glob.glob("source_images/%s/%s/*" %(part, current_session))))))//4 - 1):
                sourcefile_emotion = sorted(glob.glob("source_images/%s/%s/*" %(part, current_session)))[-(x+1)] #get path for last image in sequence, which contains the emotion
